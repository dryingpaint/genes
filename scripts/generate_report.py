...
def generate_report(result_path: str) -> str:
    ...
    # --- Traits (23andMe-style) ---
    trait_data = tools.get("traits", {}).get("output_summary", {})
    trait_list = trait_data.get("traits", [])
    health_list = trait_data.get("health", [])
    carrier_list = trait_data.get("carrier", [])

    if trait_list:
        lines.append("## Your traits")
        lines.append("")
        lines.append("These are well-studied genetic variants linked to observable traits. "
                      "Unlike disease risk, these are usually straightforward "
                      "correlations with high accuracy.")
        lines.append("")
        for t in trait_list:
            lines.append(f"- **{t['trait']}** ({t['rsid']}, genotype {t['genotype']}): "
                          f"{t['interpretation']}")
        lines.append("")

    if health_list:
        lines.append("## Health-relevant variants")
        lines.append("")
        lines.append("These variants are associated with health conditions. "
                      "Having a variant does not mean you will develop the condition — "
                      "it indicates a genetic predisposition that should be discussed "
                      "with a healthcare provider.")
        lines.append("")
        for h in health_list:
            lines.append(f"- **{h['trait']}** ({h['rsid']}, genotype {h['genotype']}): "
                          f"{h['interpretation']}")
        lines.append("")
    ...
    if carrier_list:
        ...
        lines.append("")
        lines.append("Carriers have one copy of a recessive disease variant. Carriers "
                      "are typically healthy themselves but can pass the variant to their "
                      "children. If both parents are carriers of the same condition, each "
                      "child has a 25% chance of being affected.")
        lines.append("")
        for c in carrier_list:
            lines.append(f"- **{c['trait']}** ({c['rsid']}, genotype {c['genotype']}): "
                          f"{c['interpretation']}")
        lines.append("")

    if not trait_list and not health_list and not carrier_list:
        n_checked = trait_data.get("n_variants_checked", 0)
        n_found = trait_data.get("n_found", 0)
        if n_checked:
            lines.append("## Traits & health variants")
            lines.append("")
            lines.append(f"We checked {n_checked} well-characterized trait and health variants. "
                          f"**{n_found}** were found in this sample.")
            if n_found == 0:
                lines.append("This may be because the analysis covered only chromosome 22. "
                              "A full-genome analysis would check variants across all chromosomes.")
            lines.append("")

    # --- EVEE explanations ---
    evee_data = tools.get("evee", {}).get("output_summary", {})
    evee_variants = evee_data.get("scored_variants", [])
    if evee_variants:
        explained = [v for v in evee_variants if v.get("explanations")]
        if explained:
            lines.append("## Variant explanations (AI-generated)")
            lines.append("")
            lines.append("For variants that were flagged by other tools, EVEE provides "
                          "a natural-language explanation of *why* the variant might be "
                          "important — what biological mechanism it disrupts.")
            lines.append("")
            for v in explained[:10]:
                pos = f"{v.get('chrom')}:{v.get('pos')}"
                for source, explanation in v.get("explanations", {}).items():
                    lines.append(f"- **{pos}**: {explanation}")
            lines.append("")
            lines.append("*These explanations are computational hypotheses generated by "
                          "the Evo 2 AI model, not confirmed biology.*")
            lines.append("")
